...
export default function LandingPage({ setCurrentPage }) {
  ...
  return (
    <div className="landing-page">
      ...
      {/* Main Quote submission page anchor */}
      <section id="get-quote" style={{ padding: '80px 0', backgroundColor: 'var(--bg-secondary)' }}>
        <div className="container grid-cols-2" style={{ alignItems: 'flex-start' }}>
          
          <div style={{ paddingRight: '20px' }}>
            <h2 style={{ fontSize: '36px', marginBottom: '20px' }}>Secure Elite Grades With Plagiarism-Free Answers</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '15px', marginBottom: '24px', lineHeight: '1.6' }}>
              Specify your subject, topic, and expected budget. Our system immediately matches you with the best available tutor. We double-check all answers to ensure they contain correct calculations and zero copy-paste elements.
            </p>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <span style={{ fontSize: '18px', color: 'var(--whatsapp-green)' }}>✓</span>
                <span style={{ fontSize: '14px', fontWeight: '600' }}>Direct communication with coordinators</span>
              </div>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <span style={{ fontSize: '18px', color: 'var(--whatsapp-green)' }}>✓</span>
                <span style={{ fontSize: '14px', fontWeight: '600' }}>Escrow-protected safe payment release</span>
              </div>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <span style={{ fontSize: '18px', color: 'var(--whatsapp-green)' }}>✓</span>
                <span style={{ fontSize: '14px', fontWeight: '600' }}>Strict compliance with professor guidelines</span>
              </div>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <span style={{ fontSize: '18px', color: 'var(--whatsapp-green)' }}>✓</span>
                <span style={{ fontSize: '14px', fontWeight: '600' }}>100% plagiarism-free reports</span>
              </div>
            </div>

            <div style={{ marginTop: '40px', background: '#ffffff', padding: '24px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
              <h4 style={{ fontSize: '15px', marginBottom: '8px' }}>⚡ Why WhatsApp?</h4>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>
                Emails get lost in spam and dashboards require endless logging in. WhatsApp gives you instant notifications, real-time negotiation, and direct PDF downloads on your phone.
              </p>
            </div>
          </div>

          {/* Detailed form widget */}
          <div className="glass-card" style={{ padding: '40px', border: '1px solid var(--border-color)', borderRadius: '16px', boxShadow: 'var(--shadow-xl)', background: 'white' }}>
            <h3 style={{ fontSize: '24px', fontWeight: '800', color: 'var(--primary)', marginBottom: '24px', textAlign: 'center' }}>Detailed Submission Form</h3>
            
            <form onSubmit={handleOrderSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '14px', color: '#334155', fontWeight: '700' }}>Subject Category</label>
                <select 
                  value={formData.subject}
                  onChange={(e) => setFormData({...formData, subject: e.target.value})}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '14px', color: '#334155', fontWeight: '700' }}>Topic / Assignment Title</label>
                <input 
                  type="text" 
                  placeholder="e.g. Linear Programming Lab 4"
                  value={formData.topic}
                  onChange={(e) => setFormData({...formData, topic: e.target.value})}
                  required
                  className="creative-input"
              ...
              <div className="grid-cols-2" style={{ gap: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '14px', color: '#334155', fontWeight: '700' }}>Deadline Date</label>
                  <input 
                    type="date"
                    value={formData.deadline}
                    onChange={(e) => setFormData({...formData, deadline: e.target.value})}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '14px', color: '#334155', fontWeight: '700' }}>Estimated Budget (₹)</label>
                  <input 
                    type="number"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '14px', color: '#334155', fontWeight: '700' }}>Target Grade Quality</label>
                <div style={{ display: 'flex', gap: '8px' }}>
                  {['Passing Grade', 'B Grade', 'A Grade', 'A+ Grade'].map((g) => {
                    const isActive = formData.grade === g;
                    return (
                      <button 
                        key={g}
                        type="button"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '14px', color: '#334155', fontWeight: '700' }}>Specific Instructions / Questions</label>
                <textarea 
                  rows="3" 
                  placeholder="Paste homework questions, page numbers, or guidelines here..."
                  value={formData.instructions}
                  onChange={(e) => setFormData({...formData, instructions: e.target.value})}
                  className="creative-input"
              ...
              <div 
                ...
                onClick={() => document.getElementById('detailed-file-input').click()}
                style={{
              ...
              >
                <input 
                  type="file" 
                  id="detailed-file-input" 
                  multiple 
                  style={{ display: 'none' }} 
                  onChange={handleFileChange}
                />
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#f97316" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: '8px', display: 'inline-block' }}>
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                  <polyline points="17 8 12 3 7 8"></polyline>
                  <line x1="12" y1="3" x2="12" y2="15"></line>
                </svg>
                <span style={{ fontSize: '13px', fontWeight: '700', color: '#475569', display: 'block', marginBottom: '4px' }}>
                  📎 Upload Files directly in WhatsApp Chat
                </span>
                {selectedFiles.length > 0 ? (
                  <div style={{ marginTop: '8px', fontSize: '12.5px', color: '#10b981', fontWeight: '700' }}>
                    🟢 {selectedFiles.length} file(s) selected: {selectedFiles.map(f => f.name).join(', ')}
                  </div>
                ) : (
                  <span style={{ fontSize: '11px', color: '#64748b' }}>
                    Drag & drop files or click to browse (PDFs, Images, Code sheets)
                  </span>
                )}
              </div>
              ...
              >
                {isSubmitting ? (
                  <>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" className="spinning-loader" style={{ animation: 'spin 1s linear infinite' }}>
      ...
      {/* FAQs Section */}
      <section id="faq" style={{ padding: '80px 0', backgroundColor: 'var(--bg-secondary)' }}>
